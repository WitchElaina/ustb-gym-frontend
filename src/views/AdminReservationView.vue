<template>
  <el-timeline class="reservation-timeline">
    <el-timeline-item placement="top" timestamp="选择用户" style="width: 100%">
      <el-form class="reservation-form" label-width="80px">
        <el-form-item label="用户">
          <el-select v-model="revUsername" placeholder="请选择用户" style="width: 100%">
            <el-option
        ...
        <el-form-item>
          <el-button type="primary" @click="setRevRole()">确定</el-button>
        </el-form-item>
      </el-form>
    </el-timeline-item>
      ...
      <el-form class="reservation-form" label-width="80px">
        <el-form-item label="场次">
          <el-select
            v-model="time"
            placeholder="请选择场次"
            @change="price = allPrices[time][revUserRole]"
            style="width: 100%"
          >
            <el-option
...
const allPrices = ref({});
const allUsers = ref([]);
const revUsername = ref('');
const revUserRole = ref('');
...
const setRevRole = () => {
  const user = allUsers.value.find((item) => item.username === revUsername.value);
  revUserRole.value = user.role;
  console.log('revUsername', revUsername.value);
  console.log('revUserRole', revUserRole.value);
};
  ...
  data.rounds.round.map((item) => {
    if (item.date === date.value && item.reserver === 'none') {
      item.openFor.map((roleItem) => {
        ...
        console.log(roleOpenForMap[revUserRole.value]);
        if (roleItem === roleOpenForMap[revUserRole.value]) {
          console.log(item);
...
const reserve = async () => {
  // 校验数据
  if (!room.value || !date.value || !time.value || !revUsername.value) {
